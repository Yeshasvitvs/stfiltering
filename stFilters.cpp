...
std::pair<double,double> stFilters::filtering(int& x,int& y,double& theta,double& time){


     int dx = x - center_x;
     int dy = y - center_y;

     //std::cout << " " << dx << " " << dy << " " << theta << " " << time << std::endl;//Debug Code
     ...
     double f = f_spatial * ( dx * cos(-theta) + dy * sin(-theta) );
     double even_spatial = D * gaussian_spatial  * cos(2 * M_PI * f );
     double odd_spatial = D * gaussian_spatial * sin(2 * M_PI * f );

     //Temporal Filter computation
     double gaussian_temporal = exp( - pow(time,2) / (2 * pow(var_temporal,2) )  );

     double mono_temporal = gaussian_temporal * cos( 2 * M_PI * f_temporal * time); //NOTE The negative sign for COS
     ...
     //std::cout << "Event Convolution computed: " << st_even << " "<<st_odd<< std::endl;//Debug 
